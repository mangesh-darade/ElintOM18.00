...
-- =========================================
-- 1. SMA_SECTIONS_MASTER
-- =========================================
CREATE TABLE sma_sections_master (
...
-- =========================================
-- 4. SMA_PAGE_SECTION_MAPPING
-- =========================================
CREATE TABLE sma_page_section_mapping (
    id BIGINT AUTO_INCREMENT PRIMARY KEY,
    ...
    CONSTRAINT fk_psm_page
        FOREIGN KEY (page_id)
        REFERENCES sma_pages(id)
        ON DELETE CASCADE,

    CONSTRAINT fk_psm_section
        FOREIGN KEY (section_id)
        REFERENCES sma_sections_master(id),
...
ALTER TABLE sma_page_section_mapping
...
-- =========================================
-- 5. SMA_PAGE_TAG_MAPPING
-- =========================================
CREATE TABLE sma_page_tag_mapping (
    id BIGINT AUTO_INCREMENT PRIMARY KEY,
    page_id BIGINT NOT NULL,
...
-- =========================================
-- 6. SMA_PAGE_TYPE_TAG_DEFAULTS
-- =========================================
CREATE TABLE sma_page_type_tag_defaults (
    id BIGINT AUTO_INCREMENT PRIMARY KEY,
    page_type VARCHAR(50) NOT NULL,
    tag_id BIGINT NOT NULL,
    property_name VARCHAR(100) NOT NULL,
...
-- =========================================
-- 7. SMA_GLOBAL_TAG_DEFAULTS
-- =========================================
CREATE TABLE sma_global_tag_defaults (
    id BIGINT AUTO_INCREMENT PRIMARY KEY,
    tag_id BIGINT NOT NULL,
    property_name VARCHAR(100) NOT NULL,
    value TEXT NULL,
...
-- =========================================
-- INSERT INTO sma_sections_master
-- =========================================
INSERT INTO sma_sections_master
...
-- =========================================
-- INSERT INTO sma_page_section_mapping
-- =========================================
...
-- =========================================
-- INSERT DATA INTO sma_page_tag_mapping
-- PAGE ID = 1 (HOME PAGE)
-- =========================================

INSERT INTO sma_page_tag_mapping
...
SELECT
    ...
    ptm.property_name,
    ptm.value
FROM sma_page_tag_mapping ptm
LEFT JOIN sma_tags_master stm
...
SELECT
    ...
    psm.sort_order
FROM sma_pages p
LEFT JOIN sma_page_section_mapping psm
    ON p.id = psm.page_id
LEFT JOIN sma_sections_master s
...
SELECT
    ...
    ptm.value
FROM sma_page_tag_mapping ptm
LEFT JOIN sma_pages p
    ON p.id = ptm.page_id
LEFT JOIN sma_tags_master t
    ON t.id = ptm.tag_id
ORDER BY p.id;
